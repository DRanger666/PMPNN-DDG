# Feature E Message-KPCA And Reverse-Augmentation Semantics
...
WT-center vs MT-center K-neighbor tensor bundle. It does not store one message
vector per mutation.
...
neighbor_w_neighbor_embedding                  K x 128
neighbor_m_neighbor_embedding                  K x 128
w_n_log_prob                                   K x amino-acid-log-probability-vector
m_n_log_prob                                   K x amino-acid-log-probability-vector
```
...
Important extraction nuance: the K x 128 storage shape should not be read as
"all K selected neighbors were designable in one ProteinMPNN pass." The
manuscript method text and recovered V6_V2-style code indicate a multi-pass
procedure:

```text
1. Make the center residue designable.
2. Run ProteinMPNN on the WT-centered sequence.
3. Rank/select K center-attended neighbors from the center decoder messages.
4. For each selected neighbor j:
   ...
   d. store the neighbor-j log-probability, neighbor embedding, and
      center-to-neighbor message when the directed center -> neighbor edge is
      present.
```

So the V3 pickle stores a K-row bundle per mutation entry, but the historical
candidate extraction logic appears to construct that bundle one selected
neighbor at a time. This distinction matters for interpreting randomness,
decoder-order effects, directed neighbor asymmetry, and value-level V3 direct
tensor reconciliation.

The reverse-oriented raw message-change block is generated by sign reversal of
this difference matrix:

```text
reverse_message_change[j] = -message_change[j]
```

This does not imply a separate full tensor-extraction pass for a synthetic
reverse mutation. At the RF-feature stage, the reverse row is created from the
stored paired WT/MT bundle by algebraic sign, swap, and inversion rules.
...
- The manuscript's neighbor-vector extraction section says the top-K neighbor
